...
root = tkinter.Tk()
# 指定窗口的大小
root.geometry('200x50+1500+400')
# 不允许改变窗口大小
root.resizable(False, False)
root.title("OCR")
# root.iconbitmap('icon.ico')
# 'D:\\Desktop\\My document\\0My code\\0Python\\capture2str\\icon.ico')
...
class MyCapture:
    ...
    def getText(self, type):
        with open('./temp.bmp', 'rb') as img:
            ...
            result_text = getresulttext(result, type)
            self.showtextwindow(result_text)
            # else:
            #     result = translation().en_2_zh(img)
            #     self.showtextwindow(result)
    ...
    def showtextwindow(self, text):
        ...
        # 'D:\\Desktop\\My document\\0My code\\0Python\\capture2str\\icon2.ico')
...
def getresulttext(json, type):
    ...
    if(json.__contains__('words_result_num') and json['words_result_num'] > 0):
        ...
        if type == 1:
            ...
            # s = translate().Translation_Sentence(s.replace('\r\n', ''))
            pass
    else:
        s += '未识别到文字！'
...
def key(event):
    buttonCaptureClick()
root.bind('<Control-Alt-x>', key)


# def fanyi(event):
#     buttonTranslateClick()
# root.bind('<Control-Alt-f>', fanyi)

buttonCapture = tkinter.Button(root, text='截图', command=buttonCaptureClick)
buttonCapture.place(x=15, y=10, width=80, height=30)
# buttonTranslate = tkinter.Button(root, text='翻译', command=buttonTranslateClick)
# buttonTranslate.place(x=105, y=10, width=80, height=30)
# 启动消息主循环
try:
    root.mainloop()
