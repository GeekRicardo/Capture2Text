...
class _Main:
    ...
    def main(s):
        ...
        icons = 'icon.ico'
        hover_text = "截图"  # 悬浮于图标上方时的提示
        menu_options = (('更改 图标', None, s.switch_icon),
                        ('二级 菜单', None, (('更改 图标', None, s.switch_icon),)))
        s.sysTrayIcon = SysTrayIcon(
            icons, hover_text, menu_options, on_quit=s.exit, default_menu_index=1)

        s.root.bind("<Unmap>", lambda event: s.Unmap()
                    if s.root.state() == 'iconic' else False)
        s.root.protocol('WM_DELETE_WINDOW', s.exit)
        s.root.resizable(0, 0)
        s.root.mainloop()

    def switch_icon(s, _sysTrayIcon, icons='icon2.ico'):
